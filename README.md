...
│   │   ├── account/              # Profile, sessions, usage & billing
│   │   ├── bots/                 # Bot CRUD + per-bot sub-routes:
│   │   │   └── [id]/             #   sources, channels, chat playground,
│   │   │                         #   analytics, conversations
│   │   └── dashboard/            # Overview
│   ├── api/                      # Route handlers
│   │   ├── widget/chat/          # Web widget public endpoint
│   │   ├── telegram/[id]/        # Telegram webhook
│   │   ├── whatsapp/[id]/        # WhatsApp webhook
│   │   ├── bots/[id]/            # Document upload, chat, export, conversation export
...
│   ├── dashboard/                # Sidebar, topbar, mobile drawer
│   └── theme-provider.tsx
├── db/
│   ├── schema.ts                 # Drizzle schema
│   ├── index.ts                  # Drizzle client
│   ├── migrate.ts                # CLI: enables pgvector then runs migrations
│   └── migrations/               # Generated SQL (0000–0006)
├── lib/
│   ├── auth.ts · auth.config.ts  # Auth.js v5 (Node + Edge-safe configs)
│   ├── server/                   # Server-only helpers:
│   │   ├── chat.ts               #   5-provider chat factory
│   │   ├── embed.ts              #   3-provider embed factory
│   │   ├── extract.ts            #   PDF / DOCX / XLSX / JSON / URL / sitemap extractors
│   │   ├── chunker.ts            #   Recursive character splitter
│   │   ├── ingest.ts             #   Worker pipeline
│   │   ├── chat-pipeline.ts      #   Shared chat pipeline (used by every channel)
│   │   ├── telegram.ts           #   Telegram client + group-mode helpers
│   │   ├── whatsapp.ts           #   WhatsApp client + signature verify
│   │   ├── channels.ts           #   bot_channel CRUD
│   │   ├── conversations.ts      #   Conversation admin queries
│   │   ├── promote.ts            #   Promote-to-training
│   │   ├── sessions.ts           #   Admin session management
│   │   ├── analytics.ts          #   Per-bot stats + content gaps
│   │   ├── plans.ts              #   Plan caps + quota assertions
│   │   ├── stripe.ts             #   Stripe customer + subscription sync
│   │   └── …
│   └── seo/                      # Site constants, FAQs, structured data
├── tests/                        # Vitest (64 tests, 15 suites)
├── widget/                       # Vite library — public/widget.js
├── worker/index.ts               # BullMQ ingest worker
